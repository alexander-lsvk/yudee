...
export async function uploadPropertyImage(file: File) {
  try {
    console.log("[uploadPropertyImage] Starting image upload");
    ...
    const fileName = `uploads/${Date.now()}-${Math.random()
      .toString(36)
      .substring(2)}.${fileExt}`;
    console.log("[uploadPropertyImage] Generated file name:", fileName);

    // Upload the file
    console.log("[uploadPropertyImage] Uploading to Supabase storage");
    const { error: uploadError } = await supabase.storage
    ...
    // Get the public URL
    console.log("[uploadPropertyImage] Getting public URL");
    const {
      data: { publicUrl },
    } = supabase.storage.from("property-images").getPublicUrl(fileName);

    if (!publicUrl) {
      console.error("[uploadPropertyImage] No public URL returned");
    ...
    const timestamp = Date.now();
    const urlWithCache = `${publicUrl}?t=${timestamp}`;
    console.log("[uploadPropertyImage] Generated URL:", urlWithCache);
